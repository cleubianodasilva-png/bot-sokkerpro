...
GITHUB_REPO  = os.environ.get("GITHUB_REPOSITORY", "cleubianodasilva-png/bzzoiro-bot")
...
# ═══════════════════════════════════════════════════════════════════════════════
def run():
    print("[Iniciando monitoramento — BZZOIRO]")
    sent      = load_sent()
    total_env = 0
        ...
        if precisa_fallback:
            try:
                ...
                if isinstance(sb_name, dict):
                    # Club Friendlies: sempre confiável, aceita dados normalmente
                    if "Club Friendlies" in liga:
                        stats_bzz = sb_name
